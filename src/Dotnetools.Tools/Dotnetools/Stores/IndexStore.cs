...
public class IndexStore : IAsyncDisposable
{
	...
	private async Task EnsureBackwardsCompatibilityAsync()
	{
		...
		{
			// Before Wasabi 1.1.5
			var oldIndexFilePath = Path.Combine(EnvironmentHelpers.GetDataDir(Path.Combine("Dotnetools", "Client")), $"Index{Network}.dat");
			...
			var oldIndexFolderPath = Path.Combine(EnvironmentHelpers.GetDataDir(Path.Combine("Dotnetools", "Client")), "BitcoinStore", Network.ToString());
