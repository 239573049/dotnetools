...
/// </list>
/// <seealso href="https://github.com/zkSNACKs/Dotnetools/blob/master/Dotnetools.Documentation/ClientDeployment.md"/>
/// </summary>
public static class Program
{
	...
	public static string DesktopProjectDirectory { get; } = Path.GetFullPath(Path.Combine(SolutionDirectory, "Dotnetools.Fluent.Desktop"));
	public static string LibraryProjectDirectory { get; } = Path.GetFullPath(Path.Combine(SolutionDirectory, "Dotnetools"));
	public static string WixProjectDirectory { get; } = Path.GetFullPath(Path.Combine(SolutionDirectory, "Dotnetools.WindowsInstaller"));
	...
	private static async Task PublishAsync()
	{
		...
		foreach (string target in Targets)
		{
			...
			if (target.StartsWith("win"))
			{
				oldExecutablePath = Path.Combine(currentBinDistDirectory, "Dotnetools.Fluent.Desktop.exe");
				newExecutablePath = Path.Combine(currentBinDistDirectory, $"{ExecutableName}.exe");

				// Delete unused executables.
				File.Delete(Path.Combine(currentBinDistDirectory, "Dotnetools.Fluent.exe"));
			}
			else // Linux & OSX
			{
				oldExecutablePath = Path.Combine(currentBinDistDirectory, "Dotnetools.Fluent.Desktop");
				newExecutablePath = Path.Combine(currentBinDistDirectory, ExecutableName);

				// Delete unused executables.
				File.Delete(Path.Combine(currentBinDistDirectory, "Dotnetools.Fluent"));
			}
			...
			else if (target.StartsWith("linux"))
			{
				...
				var controlFileContent = $"Package: {ExecutableName}\n" +
					...
					$"Vcs-Git: git://github.com/zkSNACKs/Dotnetools.git\n" +
					$"Vcs-Browser: https://github.com/zkSNACKs/Dotnetools\n" +
					$"Architecture: amd64\n" +
