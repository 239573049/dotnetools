...
namespace Dotnetools.Tests.UnitTests.Tor.Socks5.Pool;
...
public class TorHttpPoolTests
{
	...
	public async Task PreBuildingAsync()
	{
		...
		await using TorHttpPool pool = new(mockFactory.Object);
		pool.PrebuildCircuitsUpfront(new Uri("http://Dotnetools.io"), count: 3, deadline: TimeSpan.FromSeconds(3));
