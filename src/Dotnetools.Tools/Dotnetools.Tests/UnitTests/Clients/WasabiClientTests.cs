...
public class WasabiClientTests
{
	...
	{
		var min = int.Parse(Dotnetools.Helpers.Constants.ClientSupportBackendVersionMin);
		var max = int.Parse(Dotnetools.Helpers.Constants.ClientSupportBackendVersionMax);
		Assert.True(min <= max);

		int.Parse(Dotnetools.Helpers.Constants.BackendMajorVersion);
	}
}
